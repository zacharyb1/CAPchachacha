...
def wheel_of_fortune_captcha():
    ...
    if st.button("🎰 Spin the Wheel", key="wheel_spin"):
    ...
    if st.session_state.wheel_spinning:
        with st.spinner("Spinning the wheel..."):
            ...
            for i in range(50):  # 50 frames of spinning
                ...
                placeholder.image(rotated_wheel, caption="Spinning...")
            ...
            st.session_state.wheel_result = random.choice(non_human_answers)  # Randomly select a non-human answer
            st.session_state.wheel_spinning = False
            
            # Clear the spinning placeholder and show final result
            placeholder.empty()

    # Display the final result
    if st.session_state.wheel_result:
        final_wheel = wheel_image.rotate(-wheel_options.index(st.session_state.wheel_result) * (360 / len(wheel_options)))
        st.image(final_wheel, caption=f"Result: {st.session_state.wheel_result}")

        # Always fail regardless of the result
        st.error("❌ Error: You failed the test. Please try another method.")

    # Button to try again
    if st.button("🔄 Try Again", key="wheel_retry"):
        st.session_state.wheel_spinning = False
        st.session_state.wheel_result = None
        st.rerun()
